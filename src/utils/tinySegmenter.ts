// @ts-nocheck
// Vendored (not installed from npm) from TinySegmenter 0.2 -- Super compact Japanese
// tokenizer in Javascript. (c) 2008 Taku Kudo <taku@chasen.org>. Freely distributable
// under the terms of a new BSD licence — see http://chasen.org/~taku/software/TinySegmenter/LICENCE.txt
//
// The published npm package ("tiny-segmenter") has two bugs that break under React
// Native's strict-mode JS runtime, so this is a fixed copy rather than a dependency:
//   1. `new RegExp; regexp.compile(pattern)` uses a legacy, non-guaranteed API —
//      replaced with a plain `new RegExp(pattern)`.
//   2. `for (i = 0; ...)` in segment() assigns to an undeclared `i`, which throws
//      `ReferenceError: i is not defined` in strict mode (ES modules are always
//      strict) — fixed by declaring it.
// Data tables below are untouched.
...
export default TinySegmenter as unknown as new () => { segment(text: string): string[] };
