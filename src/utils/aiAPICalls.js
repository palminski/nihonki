...
import { imageInstructionText, singleWordInstructionText, systemInstructionText, buildGenericSystemInstructionText, buildGenericSingleWordInstructionText, buildRomanizedSystemInstructionText, buildRomanizedSingleWordInstructionText } from "./aiInstructions";
...
export async function translateWordGeneric(word, languageLabel) {
    const key = await loadAPIKeySetting();
    try {
        const openai = new OpenAI({
            apiKey: `${key}`
...
export async function translateWordRomanized(word, languageLabel, romanizationSystem) {
    const key = await loadAPIKeySetting();
    try {
        const openai = new OpenAI({
            apiKey: `${key}`
